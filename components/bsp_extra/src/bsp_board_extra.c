...
static esp_err_t codec_close_if_needed(esp_codec_dev_handle_t dev_handle, const char *name)
{
    if (!dev_handle) {
        return ESP_OK;
    }

    esp_err_t ret = esp_codec_dev_close(dev_handle);
    if (ret == ESP_OK || ret == ESP_ERR_INVALID_STATE) {
        return ESP_OK;
    }

    ESP_LOGW(TAG, "%s close failed: %s", name, esp_err_to_name(ret));
    return ret;
}
...
esp_err_t bsp_extra_codec_set_fs(uint32_t rate, uint32_t bits_cfg, i2s_slot_mode_t ch)
{
    ...
    ret |= codec_close_if_needed(play_dev_handle, "play_dev_handle");
    ret |= codec_close_if_needed(record_dev_handle, "record_dev_handle");
...
esp_err_t bsp_extra_codec_init()
{
    ...
    // Reset maximum volume curve to +0.0dB (default) to limit peak current draw and prevent screen flickering and coprocessor crashes
    ...
    esp_codec_dev_set_vol_curve(play_dev_handle, &curve);
