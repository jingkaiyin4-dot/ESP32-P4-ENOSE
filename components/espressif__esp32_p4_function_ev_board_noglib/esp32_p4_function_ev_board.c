...
static sd_pwr_ctrl_handle_t pwr_ctrl_handle = NULL; //SD LDO handle (via sd_pwr_ctrl framework)
static esp_ldo_channel_handle_t s_sd_ldo_chan = NULL; //Direct LDO handle
...
esp_err_t bsp_sdcard_sdmmc_mount(bsp_sdcard_cfg_t *cfg)
{
    ...
    // Use esp_ldo_acquire_channel directly with proper voltage (3300mV).
    // sd_pwr_ctrl_new_on_chip_ldo is NOT used because it sets voltage_mv=0,
    // causing the LDO to be configured at near-0V before SDMMC adjusts it.
    // On ESP-IDF v5.5.3 this often fails due to initial adjustable flag state.
    esp_ldo_channel_config_t ldo_cfg = {
        .chan_id = 4,
        .voltage_mv = 3300,
        .flags.adjustable = true,
    };
    esp_err_t ret = esp_ldo_acquire_channel(&ldo_cfg, &s_sd_ldo_chan);
    if (ret == ESP_OK) {
        ESP_LOGI(TAG, "LDO4 acquired directly (3300mV, adjustable)");
        cfg->host->pwr_ctrl_handle = NULL; // No SDMMC voltage adjustment, fixed 3.3V
    } else {
        ESP_LOGW(TAG, "Direct LDO4 acquisition failed (%s), SD card may not get power", esp_err_to_name(ret));
        cfg->host->pwr_ctrl_handle = NULL;
    }
    ...
    if (ret != ESP_OK) {
        ...
        if (s_sd_ldo_chan) {
            ESP_LOGW(TAG, "LDO4 stays ON (shared with SDMMC IO)");
        }
    }
    return ret;
}
...
esp_err_t bsp_sdcard_unmount(void)
{
    ...
    if (s_sd_ldo_chan) {
        ret |= esp_ldo_release_channel(s_sd_ldo_chan);
        s_sd_ldo_chan = NULL;
    }
    if (pwr_ctrl_handle) {
