...
#define WS_AIGC_URI  "wss://*.*.*.*" // Censored Volcano API / 已脱敏火山引擎API
...
static void __ws_recv_data(ws_impl_t* ws, const char* data, int data_len)
{
    ...
    LOGI("Received message type: %s, full data: %s", p_type, data);
    ...
    if (strcmp(p_type, "response.audio.delta") == 0 && p_delta) {
        ...
        LOGI("Unhandled message type: %s", p_type);
        if (strcmp(p_type, "error") == 0) {
            char* p_error_message = NULL;
            char* p_error_code = NULL;
            volc_json_read_string(p_json, "error.message", &p_error_message);
            volc_json_read_string(p_json, "error.code", &p_error_code);
            LOGE("Error from server: code=%s, message=%s", p_error_code ? p_error_code : "unknown", p_error_message ? p_error_message : "unknown");
            HAL_SAFE_FREE(p_error_message);
            HAL_SAFE_FREE(p_error_code);
        }
        if (strcmp(p_type, "session.created") == 0) {
            LOGI("%s", data);
        } else if (strcmp(p_type, "session.update") == 0) {
            LOGI("Received session.update response");
 ...
 err_out_label:
    ...
    if (p_type) {
        volc_osal_free(p_type);
        p_type = NULL;
    }
    if (p_delta) {
        volc_osal_free(p_delta);
        p_delta = NULL;
    }
    if (p_status) {
        volc_osal_free(p_status);
        p_status = NULL;
    }
    if (p_response_id) {
        volc_osal_free(p_response_id);
        p_response_id = NULL;
    }
    if (p_data) {
        volc_osal_free(p_data);
        p_data = NULL;
    }
    cJSON_Delete(p_json);
}
...
static char* __ws_generate_session_update(ws_impl_t* ws) {
    ...
    cJSON_AddNumberToObject(asr_config, "TurnDetectionMode", 0);
...
static int __ws_start(ws_impl_t* ws, volc_iot_info_t* iot_info)
{
    ...
    if (b_wait_for_session_update) {
        // wait for session.update - wait for connection to be fully established
        int connect_wait = 0;
        while (!ws->b_connected && connect_wait < 100) {
            volc_osal_thread_sleep(50);
            connect_wait++;
        }
        if (!ws->b_connected) {
            LOGE("WebSocket not connected, cannot send session.update");
            volc_ws_client_destroy(ws->client);
            ws->client = NULL;
            return -1;
        }
        
        // Send session.update with retry
        char* session_update = __ws_generate_session_update(ws);
        LOGI("Sending session.update: %s", session_update);
        if (!session_update) {
            LOGE("Failed to generate session.update");
            return -1;
        }
        
        int send_ret = -1;
        int retry_count = 0;
        while (send_ret < 0 && retry_count < 3) {
            send_ret = __ws_send_message(ws, session_update, strlen(session_update));
            if (send_ret < 0) {
                LOGW("session.update send failed, retry %d/3", retry_count + 1);
                retry_count++;
                volc_osal_thread_sleep(100);
            }
        }
        LOGI("session.update send result: %d", send_ret);
        volc_osal_free(session_update);
        
        if (send_ret < 0) {
            LOGE("session.update send failed after retries");
            volc_ws_client_destroy(ws->client);
            ws->client = NULL;
            return -1;
        }
        
        // Wait for session.update response - increase timeout to 10 seconds
        int wait_count = 0;
        while (ws->conv_status == VOLC_CONV_STATUS_ANSWER_FINISH && wait_count < 100) {
            volc_osal_thread_sleep(100);
            wait_count++;
        }
        LOGI("Session update wait done, conv_status=%d", ws->conv_status);
    }
...
static void __ws_stop(ws_impl_t* ws)
{
    ...
    // Mark as not started first to prevent other threads from accessing
    ...
    // Reset assembler
    if (ws->assembler.buffer) {
        volc_osal_free(ws->assembler.buffer);
        ws->assembler.buffer = NULL;
        ws->assembler.size = 0;
        ws->assembler.capacity = 0;
        ws->assembler.in_progress = 0;
    }
    
    // Reset response ID if exists
    if (ws->p_last_response_id) {
        volc_osal_free(ws->p_last_response_id);
        ws->p_last_response_id = NULL;
    }
    ...
    LOGI("WebSocket stopped and cleaned up");
}
