...
#define WEBSOCKET_BUFFER_SIZE_BYTE (2 * 1024)
#define WS_BUFFER_SIZE             (1 * 1600)
#define MAX_WEBSOCKET_HEADER_SIZE  16

// Increase timeout for ESP-Hosted (slower than native WiFi)
...
#define WEBSOCKET_NETWORK_TIMEOUT_MS   (30 * 1000)
#define WEBSOCKET_PINGPONG_TIMEOUT_SEC (60)    // Reduced from 540s to 60s
#define WEBSOCKET_PING_INTERVAL_SEC    (2)     // Reduced from 3s to 2s for keepalive
...
static int ws_tcp_connect(volc_ws_client_t* client, const char* host, int port, int timeout_ms)
{
    ...
    if (client->is_tls == 1) {
        ...
        client->sockfd = client->ssl->sockfd;
    } else
#endif
...
/* _ssl_write 已废弃，改为直接使用 mbedtls_client_write 封装（内部通过 esp_tls 发送）
 * 原因：旧的 _ssl_write 直接调用 mbedtls_ssl_write 绕过了 esp_tls 层，
 * 导致与 ESP-IDF v5.5 的动态缓冲区机制冲突 */
...
static int ws_connect(volc_ws_client_t* client, const char* host, int port, int timeout_ms)
{
    ...
    if (server_key == NULL) {
        LOGE("Sec-WebSocket-Accept not found, response: %s", ws->buffer);
        return -1;
    }
