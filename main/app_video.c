...
// Provide defaults only if not defined in sdkconfig
...
#define VIDEO_TASK_STACK_SIZE           (32 * 1024)  // 32KB for heavy QR processing
#define VIDEO_TASK_PRIORITY             (3)

/* Static task control block + PSRAM-allocated stack to save internal RAM */
static StaticTask_t video_stream_task_buffer;
static StackType_t *video_stream_task_stack = NULL;
...
/* Override video resolution (set before app_video_open, 0 = use default) */
uint32_t g_video_req_width = 0;
uint32_t g_video_req_height = 0;
...
int app_video_open(char *dev, video_fmt_t init_fmt)
{
    ...
    struct v4l2_format format = {
        .type = type,
        .fmt.pix.width = g_video_req_width ? g_video_req_width : default_format.fmt.pix.width,
        .fmt.pix.height = g_video_req_height ? g_video_req_height : default_format.fmt.pix.height,
        .fmt.pix.pixelformat = init_fmt,
    };

    ESP_LOGI(TAG, "Requesting resolution: %" PRIu32 "x%" PRIu32, format.fmt.pix.width, format.fmt.pix.height);

    if (ioctl(fd, VIDIOC_S_FMT, &format) != 0) {
        ESP_LOGW(TAG, "requested resolution not supported, falling back to default %" PRIu32 "x%" PRIu32,
        ...
        if (ioctl(fd, VIDIOC_S_FMT, &format) != 0) {
            ESP_LOGE(TAG, "failed to set format to %08" PRIx32, init_fmt);
            goto exit_0;
        }
    }

    // Driver might have adjusted width/height to match sensor's native capabilities. Always read back.
    if (ioctl(fd, VIDIOC_G_FMT, &format) == 0) {
        ESP_LOGI(TAG, "Negotiated width=%" PRIu32 " height=%" PRIu32, format.fmt.pix.width, format.fmt.pix.height);
    ...
    // Set frame rate to match sensor (SC2336 640x480 runs at 50 FPS)
    struct v4l2_streamparm sparm = {0};
    sparm.type = type;
    sparm.parm.capture.capability = V4L2_CAP_TIMEPERFRAME;
    sparm.parm.capture.timeperframe.numerator = 1;
    sparm.parm.capture.timeperframe.denominator = 50;
    if (ioctl(fd, VIDIOC_S_PARM, &sparm) != 0) {
        ESP_LOGW(TAG, "Failed to set stream parameters (FPS). Continuing with defaults.");
    } else {
        ESP_LOGI(TAG, "Stream parameters locked: %" PRIu32 " FPS", sparm.parm.capture.timeperframe.denominator);
    }
...
static inline esp_err_t video_stream_stop(int video_fd)
{
    ...
    int type = V4L2_BUF_TYPE_VIDEO_CAPTURE;
    int res = ioctl(video_fd, VIDIOC_STREAMOFF, &type);
    if (res) {
...
static void video_stream_task(void *arg)
{
    ...
    while (1) {
        ret = video_receive_video_frame(video_fd);
        if (ret != ESP_OK) {
            ESP_LOGE(TAG, "Failed to receive frame, retrying...");
            vTaskDelay(pdMS_TO_TICKS(100));
            continue;
        }

        if (app_camera_video.v4l2_buf.flags & V4L2_BUF_FLAG_ERROR) {
            static int err_cnt = 0;
            if (++err_cnt % 30 == 0) {
                ESP_LOGW(TAG, "Frame error! Skipping LVGL update but re-queuing buffer.");
            }
        } else {
        ...
        if (ret != ESP_OK) {
            ESP_LOGE(TAG, "Failed to free frame, retrying...");
            vTaskDelay(pdMS_TO_TICKS(100));
            continue;
        }
...
esp_err_t app_video_stream_task_start(int video_fd, int core_id)
{
    ...
    /* Allocate task stack from PSRAM to avoid internal RAM exhaustion */
    if (video_stream_task_stack == NULL) {
        video_stream_task_stack = (StackType_t *)heap_caps_malloc(VIDEO_TASK_STACK_SIZE, MALLOC_CAP_SPIRAM);
    }
    if (video_stream_task_stack == NULL) {
        ESP_LOGE(TAG, "failed to allocate video stream task stack from PSRAM");
        goto errout;
    }

    app_camera_video.video_stream_task_handle = xTaskCreateStaticPinnedToCore(
        video_stream_task, "video stream task",
        VIDEO_TASK_STACK_SIZE / sizeof(StackType_t),
        (void *)(intptr_t)video_fd, VIDEO_TASK_PRIORITY,
        video_stream_task_stack, &video_stream_task_buffer, core_id);
